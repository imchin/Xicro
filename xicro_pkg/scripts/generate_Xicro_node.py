...
def setup_var_protocol():
    ...
    try:
        for i in range (0,len(interfacetopic)):
            ...
            tempbytetograb=[]
            path = os.path.join(interfacePath(),'msg', interfacetopic[i])
...
def setupvarforcreatelib():
    ...
    try:
        ...
        for i in range (0,len(interfacefile)):
            ...
            path = os.path.join(interfacePath(),'msg',  interfacefile[i])
            msg = open(path, 'r').read().splitlines()
            for j in range(0,len(msg)):
                line=msg[j].split()
                tempType.append(line[0])
...
def gennerate(): 
    ...
    pathw= os.path.join(gPath(0)+ '/scripts/xicro_node_'+get_params("Namespace")+"_ID_"+str(id_mcu)+'.py')
